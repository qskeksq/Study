...
- 직접 기기에서 기기로 메시지를 전달할 수 없고 기기에서 보내고 싶은 토큰 값을 서버에 보내주면 서버가 그 토큰으로 원하는 기기에 메시지를 대신 전달해주는 과정을 거친다

- 파이어베이스는 기기에서 직접 서버에 접근해서 전달하지 않도록 권고하고 있다. 토큰값을 탈취당할 수 있다는 이유에서이다. 따라서 개인 서버를 만들어서 서버키를 만들고 암호화해서 서버에 전달하도록 한다.
